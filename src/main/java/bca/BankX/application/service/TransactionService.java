...
public class TransactionService {
  ...
  public Mono<Transaction> create(CreateTxRequest req) {
    System.out.println("TransactionService - Creating transaction for request: "
        + req);
    return accountRepo.findByNumber(req.getAccountNumber())
        .doOnNext(acc -> System.out.println("TransactionService - Found account"
            + " in MongoDB: " + acc))
        .switchIfEmpty(Mono.defer(() -> {
          System.out.println("Account not found in MongoDB: "
        ...
        .doOnError(e -> System.out.println("Error creating transaction: "
            + e.getMessage()));
  }

  private Mono<Transaction> validateAndApply(Account acc, CreateTxRequest req) {
    log.debug("TransactionService - Validating and applying transaction for"
  ...
  /**
   * Retrieves transactions by account.
   *
  ...
  public Flux<Transaction> byAccount(String accountNumber) {
    return accountRepo.findByNumber(accountNumber)
        ...
        .flatMapMany(acc -> txRepo.findByAccountIdOrderByTimestampDesc(
            acc.getId()));
  }
